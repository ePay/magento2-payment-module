...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    /**
     * Gives back epay configuration values
     *
     * @param  $field
     * @param null|int $storeId
     * @return mixed
     */
    public function getEpayConfigData($field, $storeId = null)
    {
        return $this->getConfigData($field, 'epay', $storeId);
    }

    /**
     * Gives back epay advanced configuration values
     *
     * @param  $field
     * @param null|int $storeId
     * @return mixed
     */
    public function getEpayAdvancedConfigData($field, $storeId = null)
    {
        return $this->getConfigData($field, 'epay_advanced', $storeId);
    }
    ...
    public function getCheckoutApi($apiName)
    {
        ...
        $api = $objectManager->create(
            'Epay\Payment\Model\Api\Checkout\\' . $apiName
        );
    ...
    public function getCheckoutApiModel($modelName)
    {
        ...
        $model = $objectManager->create(
            'Epay\Payment\Model\Api\Checkout\\' . $modelName
        );
    ...
    {
        $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
        $api = $objectManager->create('Epay\Payment\Model\Api\Epay\\' . $apiName);
    ...
    public function getEpayApiModel($modelName)
    {
        $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
        $model = $objectManager->create(
            'Epay\Payment\Model\Api\Epay\\' . $modelName
        );
    ...
    public function convertPriceToMinorunits($amount, $minorunits, $roundingMode)
    {
        ...
        switch ($roundingMode) {
            case EpayConstants::ROUND_UP:
                $amount = ceil($amount * pow(10, $minorunits));
                break;
            case EpayConstants::ROUND_DOWN:
    ...
    {
        $accesstoken = $this->getEpayCheckoutConfigData(
            EpayConstants::ACCESS_TOKEN,
            $storeId
        );
        $merchantNumber = $this->getEpayCheckoutConfigData(
            EpayConstants::MERCHANT_NUMBER,
            $storeId
        );
        $secrettoken = $this->decryptData(
            $this->getEpayCheckoutConfigData(
                EpayConstants::SECRET_TOKEN,
                $storeId
            )
        );
    ...
    public function generateEpayAuth($storeId)
    {
        ...
        $auth->merchantNumber = $this->getEpayConfigData(
            EpayConstants::MERCHANT_NUMBER,
            $storeId
        );
        $auth->pwd = $this->decryptData(
            $this->getEpayConfigData(
                EpayConstants::REMOTE_INTERFACE_PASSWORD,
                $storeId
            )
        );
    ...
    {
        return $this->_moduleList->getOne("Epay_Payment")['setup_version'];
    }
    ...
    {
        $epayVersion = $this->getModuleVersion();
        $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
        $productMetadata = $objectManager->get(
            'Magento\Framework\App\ProductMetadataInterface'
        );
        $magentoVersion = $productMetadata->getVersion();
        $result = 'Magento/' . $magentoVersion . ' Module/' . $epayVersion . ' PHP/' . phpversion(
            );
        return $result;
    }
     ...
     *
     * @param \Epay\Payment\Model\Api\Checkout\Response\Base $request
     * @param mixed $id
    ...
    public function validateCheckoutApiResult(
        ...
        if (!isset($response) || $response === false || !isset($response->meta)) {
            ...
            $this->_epayLogger->addCheckoutError($id, $message);
            return false;
     ...
     *
     * @param \Epay\Payment\Model\Api\Epay\Response\Base $response
     * @param mixed $id
    ...
    public function validateEpayApiResult($response, $id, $auth, &$message)
    {
        if (!isset($response) || $response === false) {
            //Error without description
            $message = "No answer from ePay";
            $this->_epayLogger->addEpayError($id, $message);
            return false;
            ...
            if (isset($response->epayResponse) && $response->epayResponse != -1) {
                ...
                $this->_epayLogger->addEpayError($id, "Epay Error: {$message}");
                ...
                $this->_epayLogger->addEpayError($id, "PBS Error: {$message}");
            }
            return false;
        }
        return true;
    ...
    public function createSurchargeItem(
        ...
        $feeItem->setSku(EpayConstants::EPAY_SURCHARGE);
